...
# Run first stage and reduced for estimates 
access_far_reg0 <- felm(access_percent ~ log_FAR  | county | 0 | census_tract, data=data)
usage_far_reg0 <- felm(usage_percent ~ log_FAR | county | 0 | census_tract, data=data)
access_far_reg1 <- felm(access_percent ~ log_FAR + hc_pop_den_log + hc_hu_den_log + poly(cbd_distance_miles,3) +
                 hc_workers_textile_apparel + hc_workers_eat_drink + hc_workers_other_retail + 
                 hc_workers_other_service + hc_workers_education + hc_workers_other_mfg | county | 0 | census_tract, data=data)
usage_far_reg1 <- felm(usage_percent ~ log_FAR + hc_pop_den_log + hc_hu_den_log + poly(cbd_distance_miles,3) +
                 hc_workers_textile_apparel + hc_workers_eat_drink + hc_workers_other_retail + 
                 hc_workers_other_service + hc_workers_education + hc_workers_other_mfg | county | 0 | census_tract, data=data)

# Create the table with regression results
table_str <- stargazer(access_far_reg0, access_far_reg1, usage_far_reg0, usage_far_reg1, type='latex',
        omit=c("hc_pop*", "hc_hu*", "hc_workers*", "hc_pct_families*","poly*"),
        covariate.labels=c("Log Average Commercial FAR"),
        column.separate=c(2,2),
        omit.stat=c("ser"),
        report=('vc*p'),
        add.lines=list(c("Controls:","","","",""),
                        c("Distance to City Center", " ","\\checkmark", " ", "\\checkmark"),
                        c("1960 Population and Housing Density", " ","\\checkmark", " ", "\\checkmark"),
                        c("1960 Workers by Industry", " ","\\checkmark", " ", "\\checkmark")
                       ),
          dep.var.labels = c("Access","Usage (\\% trips)")
         )


# Print the table with desired p-value format
write(gsub("p = 0.000[0-9]*", "p $<$ 0.001",paste(table_str,collapse="\n")), file="output/TableS9.tex")
