...
packages <- c("lfe", "tidyverse", "stargazer", "corrplot", "haven", "fixest", 
              "GGally", "stringr", "ggpubr")
...
data_hist_path <- file.path(main_path, "data", "clean", "cbgs_nyc_historical_zoning_data.csv")

data <- read.csv(data_path)
cbgs_nyc_historical_zoning_data <- read.csv(data_hist_path)
...
felm_ols <- felm(usage_percent ~ access_percent + pop_density_log + median_income_log + poly(cbd_distance_miles,3) + hc_pop_den_log + hc_hu_den_log + 
...
felm_iv2 <- felm(usage_percent ~ pop_density_log + median_income_log + hc_pop_den_log + hc_hu_den_log + poly(cbd_distance_miles,3) +
                   hc_workers_textile_apparel + hc_workers_eat_drink + hc_workers_other_retail + 
                   hc_workers_other_service + hc_workers_education + hc_workers_other_mfg 
                 | county | (access_percent ~ log_FAR) | census_tract, data=data)

felm_iv3 <- felm(usage_percent ~ pop_density_log + median_income_log + hc_pop_den * hc_pct_families_income_below_1k + hc_hu_den_log + poly(cbd_distance_miles,3) +
                   hc_workers_textile_apparel + hc_workers_eat_drink + hc_workers_other_retail + 
                   hc_workers_other_service + hc_workers_education + hc_workers_other_mfg + 
                   hc_pct_families_income_below_1k + hc_pct_families_nw * hc_pct_families_income_below_1k | county | (access_percent ~ log_FAR) | census_tract, data=data)
...
rownames(felm_iv2$coefficients) <- sub('.*access_percent.*', "access_percent", rownames(felm_iv2$coefficients))
rownames(felm_iv2$beta) <- sub('.*access_percent.*', "access_percent", rownames(felm_iv2$beta))
rownames(felm_iv3$coefficients) <- sub('.*access_percent.*', "access_percent", rownames(felm_iv3$coefficients))
rownames(felm_iv3$beta) <- sub('.*access_percent.*', "access_percent", rownames(felm_iv3$beta))

# Create the table with regression results
table_str <- stargazer(felm_ols, felm_iv1, felm_iv2, felm_iv3, type='latex',
        omit=c("hc_pop*", "hc_hu*", "hc_workers*", "hc_pct_families*","poly*"),
        ...
        add.lines=list(c("IV: Log Average Commercial FAR in 1961 Resolution"," ","\\checkmark","\\checkmark","\\checkmark"),
                        ...
                        c("First stage F-stat","NA",
                          ...
                          round(as.numeric(felm_iv2$stage1$iv1fstat$access_percent)[5],digits=2),
                          round(as.numeric(felm_iv3$stage1$iv1fstat$access_percent)[5],digits=2)
                         )
                       ),
